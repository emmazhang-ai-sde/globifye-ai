...
Note: pydub is NOT used here. It depends on the audioop C module removed in Python 3.13,
...
def clean_text(text: str) -> str:
    """Strip stage directions in [brackets] before sending to TTS."""
    return re.sub(r'\[.*?\]', '', text).strip()


async def generate_clip(text: str, voice: str, path: str) -> None:
    await edge_tts.Communicate(clean_text(text), voice=voice).save(path)
...
async def generate_script(script: list, output_filename: str) -> None:
    ...
    duration_estimate = sum(len(text.split()) for _, text in script) / 2.5 / 60
    print(f"  -> {out_path}  (~{duration_estimate:.1f} min)")
...
async def main():
    await generate_script(SCRIPT_1, "script-1-winning-call.mp3")
    await generate_script(SCRIPT_2, "script-2-lost-deal.mp3")
    print("\nDone.")
