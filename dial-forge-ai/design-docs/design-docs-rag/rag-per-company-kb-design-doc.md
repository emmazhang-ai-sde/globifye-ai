...
| Core-section designation | **Resolved (Step 2, Decision 1; updated 2026-07-21):** each company folder holds a `sales-playbook.md` that the tagger promotes to `core` by its section titles (Option 3); every other corpus file is `detail`. The DB now holds both — 9 `core` rows + 109 `detail`. Originally Option 1 (detail-only, playbook static in code); moved to Option 3 once Step 3's call-time retrieval needed a per-company `core` in the DB. A per-file marker is the future path only if a customer's playbook uses non-template titles. |
